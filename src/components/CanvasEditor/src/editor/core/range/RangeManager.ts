...
/**
 * 判断当前选区是否允许输入
 * 这个函数用于检查用户是否可以在当前选区位置进行输入操作
 * 主要处理了普通文本输入和控件输入的各种情况
 * 
 * @returns {boolean} 返回是否允许输入
 */
public getIsCanInput(): boolean {
  // 获取当前选区的起始和结束位置
  const { startIndex, endIndex } = this.getRange()
  
  // 如果没有有效的选区位置（startIndex 和 endIndex 都是 -1），则不允许输入
  if (!~startIndex && !~endIndex) return false
  ...
  if (startIndex === endIndex) {
    return (
      ...
      (startElement.controlComponent !== ControlComponent.PRE_TEXT ||
        elementList[startIndex + 1]?.controlComponent !== ControlComponent.PRE_TEXT) &&
