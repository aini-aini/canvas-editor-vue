...
import type { IRow, IRowElement } from '../../interface/Row'
...
export class Draw {
  ...
  // 设置打印数据，将当前编辑器内容（页眉、正文、页脚）保存到 printModeData，并过滤掉控件的辅助元素（如占位符等），用于打印模式下的数据准备
  public setPrintData() {
    this.printModeData = {
      header: this.header.getElementList(), // 获取页眉元素列表
      main: this.elementList,               // 获取正文元素列表
      footer: this.footer.getElementList()  // 获取页脚元素列表
    }
    // 过滤控件辅助元素（如控件的前后缀、占位符等），保证打印时只显示主要内容
  ...
  // 还原打印数据，将编辑器内容恢复为打印前的状态
  public clearPrintData() {
    if (this.printModeData) {
      this.setEditorData(this.printModeData) // 恢复原始数据
      this.printModeData = null              // 清空打印数据缓存
    }
  }
  ...
  public setMode(payload: EditorMode) {
    if (this.mode === payload) return // 如果模式未变，直接返回
    // 进入打印模式时，先设置打印数据
    ...
    this.clearSideEffect()         // 清除编辑器副作用（如定时器、事件等）
    this.range.clearRange()        // 清除选区
    this.mode = payload            // 设置新模式
    this.options.mode = payload    // 同步到配置
    this.render({
      isSetCursor: false,          // 渲染时不设置光标
      isSubmitHistory: false       // 不提交历史记录
    })
  }

  // 判断编辑器是否为只读状态
  public isReadonly() {
    // 如果当前激活区域（如表单区、批注区等）有自定义模式，优先判断该区域是否只读
    if (this.area.getActiveAreaInfo()?.area.mode) {
      return this.area.isReadonly()
    }
    switch (this.mode) {
      case EditorMode.DESIGN:
        return false // 设计模式下可编辑
      case EditorMode.READONLY:
      case EditorMode.PRINT:
        return true  // 只读和打印模式下不可编辑
      case EditorMode.FORM:
        // 表单模式下，只有选区在控件内时可编辑，否则只读
        return !this.control.getIsRangeWithinControl()
      default:
        return false // 其他模式默认可编辑
    }
  }

  // 判断编辑器是否为禁用状态（不可编辑、不可操作）
  public isDisabled() {
    if (this.mode === EditorMode.DESIGN) return false // 设计模式下不禁用
    const { startIndex, endIndex } = this.range.getRange()
    const elementList = this.getElementList()
    // 优先判断表格单元格是否禁用
    if (this.getTd()?.disabled) return true
    if (startIndex === endIndex) {
      // 选区为单点时，判断当前元素和下一个元素是否都禁用（如同一标题或控件内）
