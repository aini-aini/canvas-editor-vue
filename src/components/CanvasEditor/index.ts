import { commentList, data, options } from "./src/mock";
...
import { formatPrismToken } from "./src/utils/prism";
import { Signature } from "./src/components/signature/Signature";
import { debounce, nextTick, scrollIntoView } from "./src/utils";
...
const initEditor = function () {
    ...
    // 获取撤销按钮DOM元素，并设置其提示文本
    const undoDom = document.querySelector<HTMLDivElement>(".menu-item__undo")!;
    // 根据操作系统设置快捷键提示：Mac用⌘，其他用Ctrl
                    ...
                    instance.command.executeAddWatermark({
                        ...
                        gap:
                            repeat && watermark.horizontalGap && watermark.verticalGap
                                ? [Number(watermark.horizontalGap), Number(watermark.verticalGap)]
                                : undefined,
                    });
                },
            });
    ...
    const searchResultDom = searchCollapseDom.querySelector<HTMLLabelElement>(
        ".search-result"
    )!;
    ...
    const switchCatalog = () => {
        ...
        if (isCatalogShow) {
            catalogDom.classList.remove("hide");
            updateCatalog();
        } else {
            catalogDom.classList.add("hide");
        }
    };
    ...
    const paperDirectionDomOptionsDom = paperDirectionDom.querySelector<HTMLDivElement>(
        ".options"
    )!;
